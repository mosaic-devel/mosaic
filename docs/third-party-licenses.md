...
| wayland-protocols (2 XML descriptions) | 1.49 | MIT | `third_party/wayland-protocols/` (`xdg-toplevel-icon-v1.xml`, `xdg-dialog-v1.xml`, `LICENSE`, `README.md`) | protocol descriptions `wayland-scanner` turns into client stubs: the window icon a Wayland session has no other way to receive (`src/platform/wayland_toplevel_icon.cpp`) and the modal-dialog hint (`src/platform/wayland_dialog.cpp`). Vendored rather than taken from the system because the AppImage build host (Ubuntu 24.04) ships wayland-protocols 1.34 and `xdg-toplevel-icon-v1` landed in 1.36 — see that directory's README |
