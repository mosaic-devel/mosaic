...
**The Windows interface is fixed.** Every FLTK overlay that draws over the Vulkan canvas —
menus and submenus, the colour picker, flyouts, the corner panels, combo-box drop-downs, the date
picker's calendar — opened *underneath* the canvas and was simply invisible. Shaped "bubble"
flyouts drew correctly the first time and wrongly on every reopen, showing background where they
should have been transparent. Both are fixed, and the rapid flashing previously reported under
Wine is gone.

**Dark is now the default theme** on a fresh install. "System" is unchanged and one click away in
Settings, where it also follows OS theme changes and picks up the OS accent colour.

Also: the calendar pop-up in the Texture Generator could not be clicked at all — its clicks fell
through to the dialog beneath it — and Windows builds reported a bare version with no commit,
which made a build impossible to identify from a bug report. Both fixed.
...
| Platform | File | Notes |
| --- | --- | --- |
| Linux x86-64 | `Mosaic-0.3.1-linux-x86_64.AppImage` | `chmod +x`, then run |
| Linux ARM64 | `Mosaic-0.3.1-linux-aarch64.AppImage` | `chmod +x`, then run |
| Windows x64 | `Mosaic-0.3.1-windows-x86_64.msi` | per-user installer |
| Windows x64 | `Mosaic-0.3.1-windows-x86_64.zip` | no installer; unzip and run |
| Windows ARM64 | `Mosaic-0.3.1-windows-aarch64.{msi,zip}` | **never tested on hardware** |
| macOS | `Mosaic-0.3.1-macos-universal.dmg` | one image, Apple Silicon + Intel |
| Source | `Mosaic-0.3.1-source.tar.gz` | GPLv3; see README for build instructions |
...
**Linux** — `chmod +x Mosaic-0.3.1-linux-*.AppImage && ./Mosaic-0.3.1-linux-*.AppImage`.
